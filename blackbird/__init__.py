...
# noinspection PyUnresolvedReferences
from eddy.plugins.blackbird.widgets.action_explorer import BBActionWidget
# noinspection PyUnresolvedReferences
from eddy.plugins.blackbird.widgets.foreign_key_explorer import ForeignKeyExplorerWidget
# noinspection PyUnresolvedReferences
from eddy.plugins.blackbird.widgets.info import BBInfoWidget
# noinspection PyUnresolvedReferences
from eddy.plugins.blackbird.widgets.project_explorer import BlackbirdProjectExplorerWidget
# noinspection PyUnresolvedReferences
...
class BlackbirdPlugin(AbstractPlugin):
    ...
    def initActions(self):
        ...
        action = QtWidgets.QAction('BB Preferences', self, objectName='open_blackbird_preferences',
        ...
        # NODE RELATED
        #################################
        self.addAction(QtWidgets.QAction('No action available\non this table', self, objectName='empty_action',
                                         triggered=self.doNothing))
    ...
    def initWidgets(self):
        ...
        infoDockWidget.setAllowedAreas(
        ...
        actionDockWidget.setAllowedAreas(
            QtCore.Qt.LeftDockWidgetArea | QtCore.Qt.RightDockWidgetArea | QtCore.Qt.BottomDockWidgetArea)
        ...
        fkExplorerDockWidget = DockWidget('Foreign Keys Explorer', QtGui.QIcon(':icons/18/ic_explore_black'),
                                          self.session)

        fkExplorerDockWidget.addTitleBarButton(self.widget('fk_explorer_toggle'))
        fkExplorerDockWidget.installEventFilter(self)
        fkExplorerDockWidget.setAllowedAreas(
            QtCore.Qt.LeftDockWidgetArea | QtCore.Qt.RightDockWidgetArea | QtCore.Qt.BottomDockWidgetArea)
        ...
        # CREATE DOCKING AREA WIDGET
        actionTableExplorerDockWidget = DockWidget('Action Schema Tables Explorer',
        ...
        # CREATE DOCKING AREA WIDGET
        projExplorerDockWidget = DockWidget('Blackbird Project Explorer',
                                            QtGui.QIcon(':icons/18/ic_storage_black'), self.session)
    ...
    def initSchemaTableActions(self):
        ...
        for table in self.schema.tables:
            ...
            qtAction = QtWidgets.QAction(qtActionLabel, self, objectName=qtActionName,
                                         triggered=self.doShowTableDescriptionDialog)
    ...
    def doUpdateState(self):
        ...
        if (len(self.project.diagrams()) > 0 and
                (self.project.itemNum(Item.ConceptNode) > 0 or self.project.itemNum(
                    Item.RoleNode) > 0 or self.project.itemNum(Item.AttributeNode) > 0)):
                ...
                if fk.equals(oldFkEdge.foreignKey):
                    ...
                    if oldSrc in oldNodeToNew:
                        ...
                        if newNodeRelTable:
                            ...
                            newSrc = TableNode(oldSrc.width(), oldSrc.height(),
                                               remaining_characters=newNodeRelTable.name,
                                               relational_table=newNodeRelTable, diagram=newDiagram)
                            ...
                            LOGGER.debug('Problems while drawing edge {} for foreign key {} in diagram {}.\n '
                                         ...
                                         'associated to IRI {}'.format(oldFkEdge, fk.name, newDiagram.name,
                                                                       oldSrc.relationalTable.entity.fullIRI))
                    ...
                    if newSrc:
                        ...
                        if oldTgt in oldNodeToNew:
                            ...
                            if newNodeRelTable:
                                if newNodeRelTable in remSchemaTables:
                                    remSchemaTables.remove(newNodeRelTable)
                                newTgt = TableNode(oldTgt.width(), oldTgt.height(),
                                                   remaining_characters=newNodeRelTable.name,
                                                   relational_table=newNodeRelTable, diagram=newDiagram)
                        ...
                        if newTgt:
                            ...
                            newFkEdge = ForeignKeyEdge(foreign_key=fk, source=newSrc, target=newTgt,
                                                       breakpoints=oldFkEdge.breakpoints,
                                                       diagram=newDiagram)
                            ...
                            LOGGER.debug('Edge {} representing foreign key {} added to diagram {}'
                                         .format(newFkEdge, fk.name, newDiagram.name))
                        else:
                            LOGGER.debug('Problems while drawing edge {} for foreign key {} in diagram {}.'
                                         .format(oldFkEdge, fk.name, newDiagram.name))
